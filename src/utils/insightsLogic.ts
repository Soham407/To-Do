import { Agenda, DailyTask, TaskStatus, AgendaType, FailureTag } from "../types";
...
export const calculateStreak = (tasks: DailyTask[], agendaId: string): StreakInfo => {
  ...
  if (status === TaskStatus.COMPLETED || status === TaskStatus.SKIPPED_WITH_BUFFER) {
    streak++;
  ...
  let currentRun = 0;
  allTasks.forEach(t => {
     if (t.status === TaskStatus.COMPLETED || t.status === TaskStatus.SKIPPED_WITH_BUFFER) {
         currentRun++;
     } else {
...
export const getConsistencyDelta = (
    ...
    const currentTasks = tasks.filter(t => t.scheduledDate >= startStr && t.status !== TaskStatus.PENDING);
    if (currentTasks.length === 0) return 0;
    
    const currentSuccess = currentTasks.filter(t => 
        t.status === TaskStatus.COMPLETED || t.status === TaskStatus.SKIPPED_WITH_BUFFER
    ).length;
    ...
    if (prevTasks.length === 0) return 0; // No historical data
    
    const prevSuccess = prevTasks.filter(t => 
         t.status === TaskStatus.COMPLETED || t.status === TaskStatus.SKIPPED_WITH_BUFFER
    ).length;
...
export const getCorrelations = (tasks: DailyTask[], agendas: Agenda[]) => {
    ...
    const dayStats: Record<string, { tags: Set<string>, completion: number, count: number }> = {};
    ...
    tasks.forEach(t => {
       ...
       if (t.failureTag && t.failureTag !== FailureTag.NONE) ds.tags.add(t.failureTag);
    });
    ...
    if (tiredDays > 5 && normalDays > 5) {
        ...
        if (diff > 10) {
            correlations.push(`When you are '${FailureTag.TIRED}', your productivity drops by ${Math.round(diff)}%.`);
        }
    }
